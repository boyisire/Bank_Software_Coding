...
int _bmqLicense(char *aOrdername)
{
	int	i,j; 
	FILE	*fp;
	long	llCurrentTime,llAvailTime;
	struct	tm	slAvailTime;
	char	alFile[80],alBuf[200],alSignature[200],alDate[20];
	...
	if( getenv("BMQ_PATH") == NULL)
	{
		printf("读系统环境变量[SWITCH_CFGDIR]失败");
		return(-1);
	}
	...
	sprintf(alFile,"%s/etc/license.txt",getenv("BMQ_PATH"));
...
void _bmqLokiEnc( char *data )
{
  ...
  loki_enc(&lc,cp,(strlen(data)/2 + 1));
...
void _bmqLokiDec( char *data )
{
  ...
  loki_key(&lc,key);
  cp = (unsigned char *)data;
  loki_dec(&lc,cp,(strlen(data)/2 + 1));
  
}

FILE *_bmqFopen(const char *file, const char *mode)
{
...
int _swLicense(char *aOrdername)
{
	...
	if ((fp = _bmqFopen("license.txt","r")) == NULL) 
	{

...
int _xmspechk(char cTmp)
{
   if(cTmp == 0x0d || cTmp == 0x0a || cTmp == ' ') return(SUCCESS);
   return(FAIL);
}
/*检查!规则*/
int _xmkeychk(char cTmp)
...
int main(int argc,char *argv[])
{
   ...
   for(i=0;i<ilNum;i++)
   {
   	 strcpy(alXpath,"/root/bbb[2]");
     memset(alBuf,0x00,sizeof(alBuf));
     ilRc = xmlGetNode(alXpath,alBuf,NULL);
     if(ilRc)
     {
       printf("xmlGetNode [%s] Error [%s]\n",alXpath,xmlDescription(ilRc));
       exit(0);
     }

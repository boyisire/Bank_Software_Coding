...
int main(int argc, char* argv[])
{
  ...
  if( argc==1)
  {
    swVdebug(0,"S0000:启动参数错误，没有指定侦听端口号");
    return -1;
  }
  ...
  {
    swVdebug(0,"S0000:设定消息处理函数错误,:%s",strerror(errno));
    return -1;
  }
  ...
  if(ilSock<0)
  {
    swVdebug(0,"S0000:调用socket() 错误,%s\n",strerror(errno));
    return -1;
  }
  ...
  if(ilRet)
  {
    swVdebug(0,"S0000:绑定socket 到端口 %d 错误，错误消息:%s",ilPort,strerror(errno));
    return -1;
  }
  ...
  if(ilRet==-1)
  {
    swVdebug(0,"S0000:listen 函数调用错误,%s",strerror(errno));
    return -1;
  }
  ...
  while(1)
  {
    if(poll(slListen,1,1000)>0)
    {
      ...
      {
        swVdebug(1,"S0000:Accept Failed. Error:%s",strerror(errno));
      }
      else
      {
        swVdebug(1,"S0000:有新的客户端连接");
...
struct pollfd slFd[1];
  ...
  while(1)
  {
    if(poll(slFd,1,1000)>0)
    {
      if((slFd[0].revents & POLLIN )==POLLIN)
      {
        ...
        if (ilRet<=0)
        {
          swVdebug(1,"S0000:从socket 读取失败! 返回值:%d",ilRet);
          return 0;
        }
        ilRecLen+=ilRet;
  ...
  swVdebug(2,"S0000:收到客户端的请求报文:[%s]",alBuf);
  ...
  if(strcmp(alTmp,"0000")==0) /* 如果更新所有的表 */
  {
    ...
    {
      ilResult=1;
      swVdebug(1,"S0000:调用加载工具时fork() 失败.");
    }
    if(ilResult==0)
  ...
  while(ilSendLen<=strlen(alSend))
  {
    ...
    if (ilRet<=0)
    {
      swVdebug(1,"S0000:发送数据到客户端write()失败.");
      break;
      
    }
    ilSendLen+=ilRet;
  }
  swVdebug(2,"S0000:关断客户端socket.");
  close(fd);
  return 0;
}
